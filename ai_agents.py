...
class BaseAgent:
    ...
    async def make_llm_request(self,
                               prompt: str,
        ...
        try:
            ...
            response = await asyncio.to_thread(self.model.generate_content,
                                               full_prompt)
            return response.text if response.text else self.get_fallback_response(
                prompt)
        except Exception as e:
    ...
    def update_suspicion(self, target: str, level: float, reason: str):
        ...
        self.suspicions[target]['level'] = max(
            0, min(1, self.suspicions[target]['level'] + level))
        self.suspicions[target]['reasons'].append(reason)

    def build_voting_history_context(self, game_state: Dict[str, Any]) -> str:
...
class MafiaAgent(BaseAgent):
    ...
    async def choose_target(self, living_players: List[Player],
                            mafia_members: List[Player]) -> Optional[str]:
        ...
        try:
            ...
            for target in possible_targets:
                if target.name.lower() in response.lower():
                    logger.info(
                        f"Mafia {self.player.name} chose target: {target.name}"
                    )
                    return target.name
            ...
            logger.info(
                f"Mafia {self.player.name} chose random target: {target.name}")
            return target.name

        except Exception as e:
            logger.error(f"Error in Mafia target selection: {e}")
            return random.choice(
                possible_targets).name if possible_targets else None

    def build_targeting_context(self, possible_targets: List[Player],
                                mafia_members: List[Player]) -> str:
...
Based on voting patterns and previous rounds, what do you want to say in the discussion to deflect suspicion while appearing helpful?"""
    ...
    async def vote_for_elimination(self, living_players: List[Player],
        ...
        system_message = f"""You are {self.player.name}, a Mafia member. Choose who to vote for elimination. Use previous voting patterns to blend in and avoid suspicion.

Priorities:
1. Vote for players who suspect you or other Mafia
2. Follow voting trends to avoid standing out
3. Target special roles if identified
4. Never vote for fellow Mafia members
5. Use voting history to identify threats
...
Based on voting patterns and who might be targeting Mafia, who do you vote to eliminate?"""
...
Based on voting patterns and behavior, what do you want to contribute to the discussion to help find the Mafia?"""
...
- Analyze voting patterns for suspicious alliances
...
Based on voting patterns and suspicious behavior, who do you vote to eliminate?"""
        ...
        if self.suspicions:
            most_suspicious = max(self.suspicions.items(),
...
class DoctorAgent(BaseAgent):
    ...
    async def choose_save_target(
            self, living_players: List[Player]) -> Optional[str]:
...
Based on voting patterns and protecting town interests, who do you vote to eliminate?"""
        ...
        try:
            ...
            # Fallback to random vote or no vote
            import random
            return random.choice([p.name
                                  for p in possible_votes] + ["no_vote"])
        except Exception as e:
...
class DetectiveAgent(BaseAgent):
    ...
    async def choose_investigation_target(
            self, living_players: List[Player]) -> Optional[str]:
...
Who do you choose to investigate tonight?"""
        ...
        for name in possible_targets:
            if name.lower() in response.lower(
            ) and name not in self.investigation_results:
                logger.info(
                    f"Detective {self.player.name} chose to investigate: {name}"
                )
                return name

        # Fallback to most suspicious uninvestigated player
        uninvestigated = [
            name for name in possible_targets
            if name not in self.investigation_results
        ]
        if uninvestigated:
            if self.suspicions:
                # Choose most suspicious uninvestigated player
                for player, data in sorted(self.suspicions.items(),
                                           key=lambda x: x[1]['level'],
                                           reverse=True):
    ...
    def receive_investigation_result(self, target: str, is_mafia: bool):
        ...
        if is_mafia:
            self.update_suspicion(target, 1.0,
                                  "Confirmed Mafia via investigation")
        else:
            self.update_suspicion(target, -0.5,
                                  "Confirmed innocent via investigation")
...
class AgentManager:
    ...
    async def get_mafia_target(self, living_players: List[Player],
                               mafia_members: List[Player]) -> Optional[str]:
    ...
    async def get_doctor_save(self, living_players: List[Player],
                              doctor: Optional[Player]) -> Optional[str]:
    ...
    async def get_detective_investigation(
            self, living_players: List[Player],
            detective: Optional[Player]) -> Optional[str]:
    ...
    async def notify_investigation_result(self, detective: Player, target: str,
                                          is_mafia: bool):
    ...
    async def conduct_discussion(self, living_players: List[Player],
                                 game_state: Dict[str, Any]) -> str:
            ...
            if agent:
                try:
                    ...
                    discussion_statements.append(
                        f"{player.name}: I need to think about this...")

        return "\n".join(discussion_statements)

    async def conduct_voting(self, living_players: List[Player],
                             game_state: Dict[str, Any]) -> Dict[str, str]:
        ...
        for player in living_players:
            ...
            if agent:
                try:
                    vote = await agent.vote_for_elimination(
                        living_players, game_state)
                    if vote:
